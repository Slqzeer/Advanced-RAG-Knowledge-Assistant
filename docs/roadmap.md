...
**Phase 0 and steps 02-20 are done — `v1.0` is tagged. Step 22 (guardrails) is
done on its branch, and neither guardrail earned its switch. Step 21 (RAGAS) is
half done: its judge is built, tested and calibrated, and its measurement arms are
paused — the judge still calls OpenAI directly, and the OpenAI key has expired.
Finishing step 21 comes before step 23, and step 22 hands it one number it must
respect: this generator's answers drift between identical runs.**
...
| Guardrails | `app/models/answers.py`, `app/generation/{guard,llm,context}.py`, `app/evaluation/injection.py`, `data/eval/injections.json` — `Answer.refusal` (`no_context` \| `model_declined` \| `None`) replaces phrase matching for every caller. The brief's retrieval-score gate was rejected **offline**, from rows already on disk: the weakest answerable question scores 0.306 and all 7 unanswerable ones score 0.335-0.394, so the pre-registered floor catches **0 of 7**. Prompt v3 (`<entry>`-tagged context) and `detect_injection()` (3 patterns, **0 of 1 484** corpus chunks flagged) were measured on a 10 × 9 planted-chunk fixture and **neither flips**: prompt v2 already holds **79 of 80** attacks on `gemini-3.6-flash-high`, so both clause 1s were unreachable. `PROMPT_VERSION=v2`, `GUARD_DETECT=false`. The generator now goes through OmniRoute (`GENERATION_BASE_URL`), with a 60 s per-attempt timeout and the gateway's response cache bypassed on every call |
...
- **A score threshold cannot carry refusal.** The 7 out-of-corpus questions score 0.305-0.459, the answerable ones 0.341-0.664. Step 22 needs something other than a floor. **Step 22 tested it anyway, with a fixed method and no spend: the highest floor that gates no answerable question catches 0 of 7 unanswerable ones. Closed** — a later scorer can replay the rule on `results.jsonl` without a call.
- **A score threshold cannot even be shared across runs.** Every multi-query row reports `abstention_rate` **1.000** against a 0.143 baseline, and that is an artefact, not a finding: RRF replaces every cosine with `1/(k+rank)` around 0.03, so all 45 questions fall under the 0.35 floor, answerable ones included. `rrf()`'s own docstring already said this; steps 18-19 are where it bites a recorded number. The one transform row that keeps cosines is `rewrite-standalone`, which skips fusion on its single query, and there abstention moves 0.143 → **0.286** for real — a rewritten query scores lower against everything, the threshold included. Step 22 inherits both the real number and the fact that any refusal rule must be defined per scoring scheme, not per project. **No threshold shipped at step 22**, so the per-scheme rule passes, unused, to whichever later scorer tries again.
...
- **One run cannot read a ±1 clause on this generator.** Step 22 sent `gemini-3.6-flash-high` 38 byte-identical contexts twice at temperature 0: **30 of 38 answers differed and 4 refusals flipped**, a net swing of 2 — the whole margin Rule P's refusal clause was read on. Any later rule on refusals or on RAGAS scores needs a repeated arm or a measured tolerance before a tight threshold. And **a gateway's cache is not a measurement**: OmniRoute replayed 80 of one arm's 90 answers from an earlier arm until every generation call sent `X-OmniRoute-No-Cache` (`06db6d0`). A p50 latency under a second is how it showed.
...
| 22 | Phase 12 — Guardrails | `v1.6` | Refuses when retrieval is weak; survives injected instructions in documents. **Done, and the brief lost twice.** The score gate fails offline (0 of 7 caught), and every one of the 8 answerable refusals had a relevant document in context. Prompt v2 already holds 79 of 80 planted attacks, so prompt v3 and the detector have nothing to defend: both stay off. The one shipped change is `Answer.refusal`. |
...
Detailed, executable plans exist for steps 02-22:
